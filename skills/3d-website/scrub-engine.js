function mountScrollWorld(container, config) {
  ...
  if (config.brand) {
    ...
    brand.href = safeHref(config.brand.href);
  ...
  if (config.cta && config.cta.label) {
    ...
    c.href = safeHref(config.cta.href);
  ...
  function ctaBtns(cta) {
    let h = '';
    if (cta.primary)
      h += `<a class="sw-btn sw-btn--primary" href="${esc(safeHref(cta.primary.href))}">${esc(cta.primary.label)}</a>`;
    if (cta.secondary)
      h += `<a class="sw-btn sw-btn--ghost" href="${esc(safeHref(cta.secondary.href))}">${esc(cta.secondary.label)}</a>`;
    return h;
  }
}
